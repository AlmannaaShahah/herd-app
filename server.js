...
app.post("/api/register", async (req, res) => {
  ...
  const [ex] = await db.query("SELECT id FROM farmers WHERE email=?", [email]);
...
app.post("/api/login/farmer", async (req, res) => {
  ...
  if (f.status === "suspended") return res.status(403).json({ error: "حسابك موقوف — تواصل مع الإدارة" });
...
app.post("/api/animals", authFarmer, adminOnly, async (req, res) => {
  ...
  const [ex] = await db.query("SELECT id FROM animals WHERE farmer_id=? AND animal_number=?", [req.farmer_id, animal_number]);
  if (ex.length) return res.status(400).json({ error: `الرقم ${animal_number} موجود مسبقاً` });
  let mother_id = null;
  if (mother_number) {
    const [m] = await db.query("SELECT id FROM animals WHERE farmer_id=? AND animal_number=?", [req.farmer_id, mother_number]);
    if (!m.length) return res.status(400).json({ error: `رقم الأم "${mother_number}" غير موجود` });
    mother_id = m[0].id;
  }
  await db.query(`INSERT INTO animals (farmer_id,animal_number,animal_type,breed,age_label,gender,body_color,birth_date,ear_color,mother_id,notes)
    VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
...
app.put("/api/animals/:id", authFarmer, adminOnly, async (req, res) => {
  ...
  await db.query(`UPDATE animals SET animal_number=?,animal_type=?,breed=?,age_label=?,gender=?,body_color=?,birth_date=?,ear_color=?,mother_id=?,notes=?
...
app.patch("/api/animals/:id/death", authFarmer, adminOnly, async (req, res) => {
  ...
  await db.query("UPDATE animals SET status='dead',death_reason=?,death_date=? WHERE id=? AND farmer_id=?",
...
app.patch("/api/animals/:id/slaughter", authFarmer, adminOnly, async (req, res) => {
  ...
  await db.query("UPDATE animals SET status='slaughtered',slaughter_reason=?,slaughter_date=? WHERE id=? AND farmer_id=?",
...
app.delete("/api/animals/:id", authFarmer, adminOnly, async (req, res) => {
  await db.query("UPDATE animals SET mother_id=NULL WHERE mother_id=? AND farmer_id=?", [req.params.id, req.farmer_id]);
  await db.query("DELETE FROM animals WHERE id=? AND farmer_id=?", [req.params.id, req.farmer_id]);
  res.json({ success: true });
});
...
app.get("/api/report", authFarmer, async (req, res) => {
  ...
  const [breeds] = await db.query(`SELECT breed,gender,COUNT(*) cnt FROM animals WHERE farmer_id=? AND status='alive' GROUP BY breed,gender ORDER BY cnt DESC`, [fid]);
  const [dead]   = await db.query("SELECT animal_number,gender,age_label,death_reason,death_date FROM animals WHERE farmer_id=? AND status='dead' ORDER BY death_date DESC", [fid]);
  const [slau]   = await db.query("SELECT animal_number,age_label,slaughter_reason,slaughter_date FROM animals WHERE farmer_id=? AND status='slaughtered' ORDER BY slaughter_date DESC", [fid]);
  const [moms]   = await db.query(`SELECT a.animal_number,a.breed,COUNT(c.id) kids_count,MAX(c.birth_date) last_birth
  ...
  const [ready]  = await db.query("SELECT animal_number,age_label,breed,birth_date FROM animals WHERE farmer_id=? AND gender='أنثى' AND status='alive' AND age_label IN ('ثني','سديس','جامع') ORDER BY animal_number", [fid]);
  const [[k3m]]  = await db.query("SELECT COUNT(*) cnt FROM animals WHERE farmer_id=? AND mother_id IS NOT NULL AND status='alive' AND birth_date>=DATE_SUB(NOW(),INTERVAL 3 MONTH)", [fid]);
...
app.get("/api/members", authFarmer, adminOnly, async (req, res) => {
  const [rows] = await db.query("SELECT id,username,full_name,role,created_at FROM members WHERE farmer_id=?", [req.farmer_id]);
  res.json(rows);
});
...
app.post("/api/members", authFarmer, adminOnly, async (req, res) => {
  ...
  const [ex] = await db.query("SELECT id FROM members WHERE farmer_id=? AND username=?", [req.farmer_id, username]);
  if (ex.length) return res.status(400).json({ error: "اسم المستخدم موجود مسبقاً" });
  const hash = await bcrypt.hash(password, 10);
  await db.query("INSERT INTO members (farmer_id,username,full_name,password_hash,role) VALUES (?,?,?,?,?)",
    [req.farmer_id, username, full_name||username, hash, role||"member"]);
  res.json({ success: true });
});

app.delete("/api/members/:id", authFarmer, adminOnly, async (req, res) => {
  await db.query("DELETE FROM members WHERE id=? AND farmer_id=?", [req.params.id, req.farmer_id]);
  res.json({ success: true });
});

// ════════════════════════════════════════════════════════════
//  SUPER ADMIN — إدارة المزارعين
// ════════════════════════════════════════════════════════════
app.get("/api/super/farmers", authSuper, async (req, res) => {
  const [rows] = await db.query(`SELECT f.*, COUNT(a.id) animal_count
...
app.patch("/api/super/farmers/:id/status", authSuper, async (req, res) => {
  ...
  await db.query("UPDATE farmers SET status=? WHERE id=?", [status, req.params.id]);
  res.json({ success: true });
});
...
app.get("/api/super/stats", authSuper, async (req, res) => {
  const [[f]] = await db.query("SELECT COUNT(*) total, SUM(status='active') active, SUM(status='pending') pending FROM farmers");
  const [[a]] = await db.query("SELECT COUNT(*) total FROM animals");
